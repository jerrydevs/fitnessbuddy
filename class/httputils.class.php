...
{
  public static function redirect($uri, $http_status = 302)
  {
    global $CFG;

    // NOTE: Web redirects need to use the absolute URL so
    //       $CFG->base_url is used for this...

    // Suffix base_url with a '/' if needed...
    if (
      strlen($CFG->base_url) > 0 
      && $CFG->base_url[strlen($CFG->base_url)-1] != '/'
      && strlen($uri) > 0
      && $uri[0] != '/'
    )
    $CFG->base_url .= '/';

    header('Location: '.$CFG->base_url.$uri, true, $http_status);
    exit();
  }
